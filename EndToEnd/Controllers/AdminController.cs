...
namespace EndToEnd.Controllers
{
    public class AdminController : Controller
    {
        ...
        // GET: /Admin/Details
        [Authorize(Roles = "Administrator")]
        #region public ActionResult Details(string UserName)
        ...
        [Authorize(Roles = "Administrator")]
        #region public ActionResult Groups()
        public ActionResult Groups()
        {
            var context = new ApplicationDbContext();
            var query = from p in context.Groups
                        
                        select new { Name = p.GroupName };
            var colgroups = query.ToList().Select(r => new Groups
            {
                GroupName = r.Name,
            }).ToList();

            return View(colgroups);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        #region public ActionResult AddGroups(Groups paramRoleDTO)
        public ActionResult AddGroups(Groups paramRoleDTO)
        {
            try
            {
                ...
                var GrpName = paramRoleDTO.GroupName.Trim();

                if (GrpName == "")
                {
                    throw new Exception("No Team Name");
                }
                ...
                if (!roleManager.RoleExists(GrpName))
                {
                    roleManager.Create(new IdentityRole(GrpName));
                }
        ...
        public object GroupName { get; private set; }
        #endregion
